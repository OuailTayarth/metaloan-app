...
contract MetaPayment is ReentrancyGuard {
  // Events
  event PlanCreated(
   ...
   *
   **/
  function deleteLoan(uint256 _planId) external LoanExists(_planId) {
  ...
  function deletePlan(
    uint256 _planId
  ) external PlanExists(_planId) onlyManager onlyOwner {
    require(deleteTime, "MetaLoan :: deleteTime loan is not activated yet");
